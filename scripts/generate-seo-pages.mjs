...
<body>
<header class="nav"><a class="brand" href="/">⚡ Tagshield</a>
<nav><a href="/#how">How it works</a><a href="/#pricing">Pricing</a><a class="btn btn-ghost" href="/#scan">Scan my site</a></nav></header>
<main>
<section class="hero" style="padding:40px 0 18px;text-align:left">
<h1 style="font-size:clamp(26px,5vw,40px)">${esc(h1)}</h1>
</section>
${bodyHtml}
<section class="scan" style="margin-top:30px">
...
<a class="btn btn-primary" href="/#scan">Scan my site free →</a>
</section>
</main>
<footer class="foot"><p>⚡ Tagshield — consent that keeps your ads running.</p>
<p class="muted">Tagshield is a tool, not legal advice. Always confirm your obligations with counsel.</p></footer>
</body></html>`;
}
...
function buildStates() {
  ...
  for (const s of states) {
    ...
    const body = `
<section class="how">
<p>The <strong>${esc(s.law)}</strong> took effect on <strong>${esc(s.effective)}</strong>. If you run
...
<p>Tagshield resolves the ${esc(s.name)} rules at the edge automatically, honors GPC, and logs every
...
function buildPlatforms() {
  ...
  for (const p of platforms) {
    const body = `
<section class="how">
<p>Add a Google Consent Mode v2 cookie banner to your ${esc(p.name)} site in about 10 minutes.</p>
<ol><li>${esc(p.install)}</li>
...
<div class="install"><pre><code>&lt;script&gt;
  window.__tagshield = { key: "YOUR_SITE_KEY" };
  // sets Consent Mode v2 defaults, then loads the &lt;10KB banner
&lt;/script&gt;</code></pre></div>
<p>Because the banner is sub-10KB and edge-delivered, it won't move your ${esc(p.name)} Core Web Vitals.</p>
</section>`;
...
function buildVs() {
  ...
  for (const c of comps) {
    ...
    const body = `
<section class="bullets" style="grid-template-columns:1fr 1fr">
<article><h3>${esc(c.name)} (${esc(c.pricing)})</h3><ul class="muted">${weak}</ul></article>
<article><h3>Tagshield</h3><ul>${edge}</ul></article>
</section>
<section class="how"><p>Switching from ${esc(c.name)}? Tagshield offers free migration and flat per-visitor
pricing across every site, with a tamper-evident consent vault you can export. No quote call, no
Core Web Vitals hit.</p></section>`;
